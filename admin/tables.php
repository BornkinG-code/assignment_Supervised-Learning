...
<div class='container admin-shell'>
  <header class='admin-topbar card'>
    <div><div class='brand'>Table Management</div><div class='subtle'>Manage dine-in table activity and QR links</div></div>
    <a class='btn btn-ghost' href='index.php'>Back to Dashboard</a>
  </header>

  <section class='card'>
    <div class='card-body'>
      <form id='tableForm' class='toolbar'>
        ...
        <button class='btn btn-add'>Save Table</button>
      </form>
...
async function load(){
  ...
  rows.innerHTML=tableCache.map(t=>`<tr>
    <td>${t.table_name}</td>
    <td>${t.qr_path?`<a href='../${t.qr_path}' target='_blank' download>Download QR</a>`:'<button class="btn btn-ghost" onclick="regen('+t.id+')">Generate QR</button>'}</td>
    <td>${t.is_active==1?'Yes':'No'}</td>
    <td>
      <button class='btn btn-ghost' onclick='edit(${t.id})'>Edit</button>
      <button class='btn btn-ghost' onclick='regen(${t.id})'>Regenerate QR</button>
      <button class='btn btn-danger' onclick='delTable(${t.id})'>Delete</button>
    </td>
  </tr>`).join('');
}
function edit(idVal){
